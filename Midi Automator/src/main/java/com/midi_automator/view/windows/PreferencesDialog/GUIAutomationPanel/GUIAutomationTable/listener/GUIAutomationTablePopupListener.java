 ...
 */
@Component
public class GUIAutomationTablePopupListener extends MouseAdapter {
	...
	private void maybeShowPopup(MouseEvent e) {
		...
		GUIAutomationTable table = (GUIAutomationTable) e.getSource();
		...
		if (column == table
				.getColumn(GUIAutomationTable.COLNAME_MIDI_SIGNATURE)
				.getModelIndex()) {
			table.openMidiLearnPopupMenu(table, e.getX(), e.getY());
		}

		if (column == table.getColumn(GUIAutomationTable.COLNAME_KEYS)
				.getModelIndex()) {
			table.openKeyLearnPopupMenu(table, e.getX(), e.getY());
		}

		if (column == table.getColumn(GUIAutomationTable.COLNAME_IMAGE)
				.getModelIndex()) {
