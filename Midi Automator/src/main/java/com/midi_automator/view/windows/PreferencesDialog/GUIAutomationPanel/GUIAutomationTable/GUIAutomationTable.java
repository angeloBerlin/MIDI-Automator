...
import com.midi_automator.view.windows.PreferencesDialog.CacheableJTable;
...
import com.midi_automator.view.windows.PreferencesDialog.GUIAutomationPanel.GUIAutomationTable.listener.GUIAutomationTablePopupListener;
import com.midi_automator.view.windows.PreferencesDialog.GUIAutomationPanel.GUIAutomationTable.listener.KeyLearnListener;
import com.midi_automator.view.windows.PreferencesDialog.GUIAutomationPanel.GUIAutomationTable.listener.TriggerComboBoxListener;
...
public class GUIAutomationTable extends CacheableJTable {
	...
	public static final String NAME_COMBOBOX_TRIGGER_EDITOR = "COMBOBOX_TRIGGER_EDITOR";
	...
	@Autowired
	private GUIAutomationTablePopupListener popupListener;
	@Autowired
	private TriggerComboBoxListener triggerComboBoxListener;
	...
	private void createTriggerColumn() {
		...
		JComboBox<String> triggerComboBox = new JComboBox<String>(
				triggerTypes.toArray(new String[0]));
		triggerComboBox.addActionListener(triggerComboBoxListener);
	 ...
	 */
	public void openKeyLearnPopupMenu(JTable table, int x, int y) {
	 ...
	 *            The y coordinate of the menu
	 */
	public void openImagePopupMenu(JTable table, int x, int y) {

		int row = table.rowAtPoint(new Point(x, y));

		// de-/activate remove image item
		ScaleableImageIcon image = (ScaleableImageIcon) table.getValueAt(row,
				getColumn(COLNAME_IMAGE).getModelIndex());
	 ...
	 *            The y coordinate of the menu
	 */
	public void openMidiLearnPopupMenu(JTable table, int x, int y) {
		...
		if (midiSignature != null) {
			if (!midiSignature.equals("")) {
				midiLearnPopupMenu.getMidiUnlearnMenuItem().setEnabled(true);
			}
		}
